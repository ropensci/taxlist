...
for handling taxonomic data i **R**.
...
# Extracting a species list from a vegetation table
...
Note that the function `summary` provides a quick overview in the content of the
resulting object.

## Resolving taxonomic names with taxize

One of the main concerns is to resolve nomenclatorial
issues, especially when working with historical data or combining data from
different sources.
For this purpose, `taxlist` implements a method for the function `tnrs` (package
`taxize`):

```{r resolving_nomenclature, message=FALSE}
summary(Splist)
...
taxon names increased, indicating that some of the names in the original source
are currently considered as synonyms.
Check at the first entries in `Splist`:
...
ways to replace names by accepted ones, in `taxlist` objects both, accepted
names and synonyms are preserved.
...
## Example of a phytosociological classification
