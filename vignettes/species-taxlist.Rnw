...
below).
By this output we get the ID of the concept (52341) and the ID of the new name
(53193), which correspond to the fields \texttt{TaxonConceptID} and
\texttt{TaxonUsageID}, respectively.
...
Note that this function is only working only for one concept, thus all names
inserted will be added as synonyms of the indicated concept:
...
Note that the function \texttt{summary} with the object alone produce a
summarizing statistic of the object, while an indication of the concept ID (as
\texttt{integer}) will provide the full overview of the respective concept.
You also have the option to use the keyword \texttt{\textquotedbl
all\textquotedbl} instead of the concept ID and then you will get the overview
of all concepts included in the object.

\subsection{Subset building}

As in the case of the function \texttt{summary}, there are specific methods
implemented for the function \texttt{subset} to \texttt{taxlist} objects.
To get the documentation of the function use the command
\texttt{?taxlist::subset} in the R console.
Previous to use the function subset, we will recommend a detailed review on
logical and pattern matching functions.
There are three ways to produce subsets that are using information from three
different slots in the taxlist objects.
...
\section{Merging and splitting concepts}
