...
#' Cyperus2 <- taxmap2taxlist(
#'   taxmap = Cyperus2, traits = "traits", views = "views",
#'   synonyms = "synonyms"
#' )
#' Cyperus2
...
#'
setMethod(
  "taxlist2taxmap", signature(taxlist = "taxlist"),
  function(taxlist, ...) {
    ...
    obj$data$synonyms <- synonyms(taxlist)[, c(
      "TaxonConceptID",
      "TaxonUsageID", "TaxonName", "AuthorName"
    )]
    names(obj$data$synonyms) <- c(
      "taxon_id", "TaxonUsageID", "synonym",
      "synonym_authority"
    )
...
taxmap2taxlist <- function(taxmap, relations, traits, synonyms, views,
  ...
  if (reindex) {
    TaxonConceptID <- as.integer(seq_along(old_ids))
  } else {
    TaxonConceptID <- as.integer(old_ids)
  }
  if (any(is.na(TaxonConceptID))) {
    stop(paste(
      "Some values of 'taxon_id' cannot be coerced to integers.",
      "Try 'reindex=TRUE'"
    ))
  }
  ...
  taxonRelations$Parent <- TaxonConceptID[match(
    taxonRelations$Parent,
    old_ids
  )]
  ...
  if (!missing(synonyms)) {
    ...
    colnames(Synonyms) <- taxlist::replace_x(
      colnames(Synonyms),
