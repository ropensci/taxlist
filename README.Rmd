...
the family Cyperaceae and then extract only species.
...
Now, we can count the number of species in each genus of the family Cyperaceae
and display them in a bar plot:
...
Such kind of calculations can be also implemented in functions once working
with biodiversity records, for instance in the functions `trait_stats()` and
`trait_proportion()` of the package
`[vegtable](https://github.com/kamapu/vegtable)`.
