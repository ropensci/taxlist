...
#'
setMethod(
  "df2taxlist", signature(x = "data.frame", AcceptedName = "logical"),
  function(x, AcceptedName, levels, ...) {
    ...
    if (length(AcceptedName) == 1) {
      if (!AcceptedName) {
        stop(paste(
          "for 'AcceptedName' of length 1 only value",
          "'TRUE' is allowed"
        ))
      }
...
#'
setMethod(
  "df2taxlist", signature(x = "data.frame", AcceptedName = "missing"),
  function(x, ...) {
...
#'
setMethod(
  "df2taxlist", signature(x = "character", AcceptedName = "missing"),
  function(x, ...) {
    if (any(duplicated(x))) {
    ...
    x <- list(
      TaxonUsageID = seq_along(x), TaxonConceptID = seq_along(x),
      TaxonName = x, ...
    )
