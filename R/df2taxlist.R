...
df2taxlist <- function(x, ValidName, FirstName, Traits) {
    # Some tests previous to run the function
	if(!is.data.frame(x))
		stop("'x' must be of class 'data.frame'", call.=FALSE)
    if(missing(ValidName))
        stop("'ValidName' is a mandatory argument", call.=FALSE)
    ValidName <- substitute(ValidName)
    ValidName <- eval(ValidName, x, parent.frame())
    if(length(ValidName) != nrow(x))
        stop("Argument 'ValidName' not matching the size of 'x'")
    ...
    # taxon relations
    taxonRelations <- x[ValidName,c("TaxonConceptID","TaxonUsageID")]
    colnames(taxonRelations)[2] <- "ValidName"
