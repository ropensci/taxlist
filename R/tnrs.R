...
#'
setMethod(
  "tnrs", signature(query = "character"),
...
#'
setMethod(
  "tnrs", signature(query = "taxlist"),
  function(query, min_score = 0.8, source = "iPlant_TNRS", ...) {
    spp_out <- taxize::tnrs(query@taxonNames$TaxonName,
      source = source,
      ...
    )
    # score for subset
    ...
    query <- add_synonym(
      query, spp_out[spp_out$new, "TaxonConceptID"],
      spp_out[spp_out$new, "acceptedname"],
      spp_out[spp_out$new, "authority"]
    )
    # correct names
    query@taxonNames$TaxonName[na.omit(match(
      spp_out$submittedname,
    ...
    query@taxonNames$AuthorName[na.omit(match(
      spp_out$TaxonUsageID,
      query@taxonNames$TaxonUsageID
    ))] <-
      spp_out$authority
    # Complete entries in the species list
    query@taxonNames$Link <- spp_out[
      match(
